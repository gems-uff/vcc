...
Existem diversas tarefas em Mineração de Dados, as principais \cite{Goncalves}, que serão apresentadas nesse capítulo são: classificação, regras de associação, clusterização, padrões em séries temporais e padrões seqüências. Em geral, essas tarefas podem ser classificadas em duas categorias: mineração preditiva e mineração descritiva \cite{LivroMineracao}.
...
Na mineração descritiva, padrões e regras descrevem características importantes dos dados dos quais se está trabalhando. As tarefas de regras de associação, clusterização e padrões sequenciais fazem parte dessa categoria.
...
\subsubsection{Regras de Associação}
	...
	Uma regra de associação representa um padrão de relacionamento entre itens de dados do domínio da aplicação, que ocorre com uma determinada frequência. Essas regras são extraídas a partir de uma base de dados organizada em transações, que são formadas por um conjunto de itens desse domínio. 
	Um exemplo fictício de regra de associação que poderia ser extraído de uma base de dados de dados qualquer de vendas é: clientes que compram o produto A geralmente compram o produto B. 
	
	Outro exemplo, porém real, que foi extraído de uma base de dados médica após a aplicação dessa técnica, descreve que pacientes aidéticos que contraem a doença candíase também têm pneumonia.


\subsubsection{Clusterização}

A tarefa de clusterização é usada para agrupar elementos de uma base de dados através de seus atributos ou características, de forma que elementos mais similares fiquem no mesmo cluster e elementos menos similares fiquem em clusters distintos. 
...
\subsubsection{Padrões em Séries Temporais}

	Consiste de uma seqüência de valores mensurados em intervalos iguais de tempo \cite{LivroMineracao}. O principal objetivo da análise de padrões sequenciais em séries temporais é realizar previsões futuras baseando-se no histórico dos dados. 
...
Podemos dizer que um evento em uma base de dados de uma loja de vendas é uma compra feita por um cliente, e o item do domínio da aplicação são os produtos que pertencem à essa compra. 
...
\cite{Zimmerman} aplica técnicas de mineração de dados em repositórios versionados de código fonte para extrai regras de associação do tipo: "`programadores que alteraram a função A também alteraram as funções B e C"'. Tais regras são extraídas com o objetivo de guiar desenvolvedores de software na alteração do código fonte.
