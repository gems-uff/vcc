...
\section{A Análise do Código Fonte Através da ASTParser}
	...
	Para extrair os padrões sequenciais frequentes da aplicação em que o VCC está sendo utilizado, é preciso obter todas as chamadas de métodos utilizadas nos corpos dos métodos desse projeto. Conforme mostra a Figura \ref{fig:tela-menu}, o usuário deve clicar no item do menu \textit{Generate Tree} para que a árvore de padrões frequentes possa ser gerada. 
...
\section{Mineração do Código Fonte}
 ...
 Existem diversas implementações de código aberto de algoritmos de mineração de padrões sequenciais, disponíveis na literatura. Por esse motivo, foi decidido que não seria necessário o desenvolvimento de um programa que realize a mineração desses padrões. Após a análise dos algoritmos GSP~\cite{SrikantAgrawal}, PLWAP~\cite{Ezeife} e WapTree~\cite{Zhou}, o PLWAP foi escolhido como o algoritmo que seria utilizado para a extração dos padrões por apresentar uma melhor performance com relação aos demais.
...
Conforme visto no Capítulo 3, todos os padrões sequenciais são armazenados em uma árvore, que é consultada quando um padrão sequencial está sendo buscado. Na implementação do VCC, esta árvore é representada por um conjunto de nós, que são objetos da classe \textbf{MethodCallNode}. Essa classe possui além das confianças do padrão e da assinatura completa do método, uma referência para o nó pai e um \textbf{HashMap}, que contém todos os filhos desse nó. Um \textbf{HashMap} é uma classe Java, que através de uma tabela \textit{hash}, organiza um mapa de dados~\cite{Myers}.
...
\section{Geração e Poda das Combinações de Chamadas de 
\newline Métodos}
...
A primeira dessas opções é analisar todas as chamadas de métodos que estão disponíveis no corpo do método que está sendo codificado. A seleção desse método poderia ser feita através da digitação do nome do pacote, da classe e do próprio método. Embora essa opção represente uma facilidade na implementação, prejudica em muito a usabilidade da ferramenta, além de não permitir uma melhor delimitação da área de consulta. Um usuário poderia realizar uma alteração na metade do corpo do método, por exemplo, e independentemente disso, todas as chamadas, do início ao fim do mesmo seriam utilizadas na pesquisa por padrões de código fonte.
...
A Figura \ref{fig:print-search-patterns} mostra a posição do cursor no corpo do método do usuário, no momento da requisição de obtenção das sugestões de código fonte.

\begin{figure}[htb]
       \centering  % figura centralizada
       \fbox{\includegraphics[scale=0.3]{print-search-patterns.png}}
       \FigLegenda{\label{fig:print-search-patterns} Chamada do usuário com o intuito de obter as sugestões de código fonte.}
\end{figure}
